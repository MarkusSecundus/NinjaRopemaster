...
@export var ease : EaseType.Enum = EaseType.Enum.LINEAR
...
func _move_next(indexer: CoroutineUtils.Generator)->void:
	...
	Tweens.tween(_platform, "position", begin, end, duration, EaseType.from_enum(ease), func(t:CustomTween):_move_next(indexer));
